...
exports.mergeBuiltLibraries = function() {
    ...
    return fs.readdir(builtLibrariesPath).then(dirlist => {
        Promise.all(
            dirlist.map(
                dirName => exports.mergeContent(path.join(builtLibrariesPath, dirName), destinationPath, ""),
                ""
            )
        );
    });
                ...
                .then(contentlist =>
                    Promise.all(
                        contentlist.map(subContentPath =>
                            exports.mergeContent(libraryPath, destinationPath, path.join(contentPath, subContentPath))
                        )
                    )
                );
        } else {
...
exports.copyContent = function(libraryPath, destinationPath, contentPath) {
    ...
    return (existsFile(destinationDirectory)
